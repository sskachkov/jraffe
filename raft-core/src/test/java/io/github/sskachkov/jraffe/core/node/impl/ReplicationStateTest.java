...
class ReplicationStateTest {
    ...
    void initialStateIsOptimisticNextIndexAndZeroMatchIndex() {
        ReplicationState tracker = new ReplicationState(List.of("p1", "p2"), 10, 1);
    ...
    void constructorSeedsStalenessBaselineFromCounterSoOldReqIdsAreIgnored() {
        ...
        ReplicationState tracker = new ReplicationState(List.of("p1"), 0, 5);
    ...
    void recordSuccessAdvancesNextIndexAndMatchIndex() {
        ReplicationState tracker = new ReplicationState(List.of("p1"), 0, 0);

        tracker.recordSuccess("p1", 1, 7, 0);
    ...
    void recordFailureDecrementsNextIndexButPreservesMatchIndex() {
        ReplicationState tracker = new ReplicationState(List.of("p1"), 10, 0); // nextIndex starts at 11

        tracker.recordSuccess("p1", 1, 5, 0); // matchIndex=5, nextIndex=6
        tracker.recordFailure("p1", 2, 0);
    ...
    void recordFailureNeverGoesBelowOne() {
        ReplicationState tracker = new ReplicationState(List.of("p1"), 0, 0); // nextIndex starts at 1

        for (int i = 0; i < 5; i++) {
            tracker.recordFailure("p1", i + 1, 0);
        }
    ...
    void majorityMatchIndexForOddClusterSize() {
        ...
        ReplicationState tracker = new ReplicationState(List.of("p1", "p2", "p3", "p4"), 0, 0);

        tracker.recordSuccess("p1", 1, 8, 0);
        tracker.recordSuccess("p2", 2, 6, 0);
        tracker.recordSuccess("p3", 3, 4, 0);
        tracker.recordSuccess("p4", 4, 2, 0);
    ...
    void majorityMatchIndexForEvenClusterSize() {
        ...
        ReplicationState tracker = new ReplicationState(List.of("p1", "p2", "p3"), 0, 0);

        tracker.recordSuccess("p1", 1, 8, 0);
        tracker.recordSuccess("p2", 2, 6, 0);
        tracker.recordSuccess("p3", 3, 2, 0);
    ...
    void majorityMatchIndexCountsTheLeadersOwnIndex() {
        ...
        tracker.recordSuccess("p1", 1, 20, 0); // peer is way ahead of the leader
