package io.github.sskachkov.jraffe.core.logging;
...
/**
 * Logger decorator that prefixes every message with a fixed context tag (e.g. node id).
 * Only the classic SLF4J methods carry the prefix - fluent atDebug()/atInfo()/etc. calls are
 * inherited undecorated.
 */
public class ContextAwareLogger implements Logger {
    private final Logger delegate;
    private final String prefix;

    public ContextAwareLogger(Class<?> clazz, String context) {
        this(clazz, List.of(context));
    }

    public ContextAwareLogger(Class<?> clazz, List<String> context) {
        this.delegate = LoggerFactory.getLogger(clazz);
        this.prefix = buildPrefix(context);
    }

    private static String buildPrefix(List<String> context) {
        if (context.isEmpty()) {
            return "";
        }
